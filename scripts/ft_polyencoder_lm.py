...
from models.polyencoder import PolyEncoderTokenizer, PolyEncoderLM
...
class TextDataset(Dataset):
    def __init__(self, tokenizer: PolyEncoderTokenizer, args, file_path: str, block_size=512):
        assert os.path.isfile(file_path)

        block_size = block_size - 2
        directory, filename = os.path.split(file_path)
        cached_features_file = os.path.join(directory, "polyencoder_cached_lm_" + str(block_size) + "_" + filename)
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            bos_token_id = tokenizer.dict[tokenizer.dict.start_token]
            eos_token_id = tokenizer.dict[tokenizer.dict.end_token]
...
def evaluate(args, model, tokenizer: PolyEncoderTokenizer, prefix="") -> Dict:
...
def main():
    ...
    parser.add_argument(
        "--num_train_epochs", default=2.0, type=float,
    ...
    model = PolyEncoderLM.from_pretrained()
    ...
    if args.do_eval:
        ...
        for checkpoint in checkpoints:
            ...
            model = PolyEncoderLM.from_pretrained(checkpoint=checkpoint)
            model.to(args.device)
